...
import type { ReleaseRailConfig } from "./types.js";
...
async function latestReleaseTag(client: GitHubClient, config: ReleaseRailConfig): Promise<string | null> {
...
async function loadConfig(client: GitHubClient, path: string, ref: string): Promise<ReleaseRailConfig> {
  const content = await client.getFile(path, ref);
  return content ? parseConfig(content, path) : parseConfig("");
}

function releasePrBody(plan: WorkspaceReleasePlan, config: ReleaseRailConfig): string {
...
async function runReadinessCommands(config: ReleaseRailConfig): Promise<Record<string, boolean>> {
...
async function runReleaseHealth(client: GitHubClient, releaseEvent: NonNullable<ReleaseEvent["release"]>, config: ReleaseRailConfig): Promise<void> {
...
async function prepareRelease(client: GitHubClient, head: string, config: ReleaseRailConfig): Promise<void> {
...
function isReleaseRailReleasePullRequest(pr: GitHubPullRequest, config: ReleaseRailConfig): boolean {
...
function independentTagName(config: ReleaseRailConfig, packageItem: PublishedPackage): string {
  const safeName = packageItem.name.replace(/^@/, "").replace(/[\\/]/g, "-");
  return `${config.release.independentTagPrefix}${safeName}@${packageItem.version}`;
}

function packageTagName(config: ReleaseRailConfig, mode: ReleaseRailManifest["mode"], packageItem: PublishedPackage): string {
...
async function publishRelease(client: GitHubClient, pr: GitHubPullRequest, config: ReleaseRailConfig): Promise<void> {
  ...
  const manifest: ReleaseRailManifest = manifestContent ? JSON.parse(manifestContent) as ReleaseRailManifest : {};
