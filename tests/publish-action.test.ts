...
describe("merged release publication", () => {
  ...
  it("creates a tag and GitHub release from the merged release PR", async () => {
    const directory = mkdtempSync(join(tmpdir(), "shipkit-publish-"));
    ...
    for (const [key, value] of Object.entries({ GITHUB_API_URL: "https://api.github.test", GITHUB_REPOSITORY: "demo/repo", GITHUB_EVENT_NAME: "pull_request", GITHUB_SHA: "merge-sha", GITHUB_EVENT_PATH: eventPath, GITHUB_OUTPUT: outputPath, INPUT_GITHUB_TOKEN: "test-token", INPUT_CONFIG: ".shipkit.yml" })) {
