...
describe("published release health action", () => {
  ...
  it("checks expected assets and configured publishing workflows", async () => {
    const directory = mkdtempSync(join(tmpdir(), "releaserail-health-"));
    ...
    for (const [key, value] of Object.entries({ GITHUB_API_URL: "https://api.github.test", GITHUB_REPOSITORY: "demo/repo", GITHUB_EVENT_NAME: "release", GITHUB_SHA: "merge-sha", GITHUB_EVENT_PATH: eventPath, GITHUB_OUTPUT: outputPath, INPUT_GITHUB_TOKEN: "test-token", INPUT_CONFIG: ".releaserail.yml" })) {
